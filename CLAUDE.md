...
```
Step 10 완료 — 레이스 결과 화면 구현 완료.
다음: Step 10b (동팀 드라이버 시각 구분) → Step 11 (트랙 맵 고도화) 순.
```

### 다음 AI가 할 일 (Step 10b → Step 11)

#### Step 10b: 동팀 드라이버 시각 구분 (Option C)

**문제**: 같은 팀 두 드라이버 비교 시 라인 색상이 동일해 구분 불가.

**해결 방향**: Driver A = 원래 팀 컬러 + 실선 / Driver B = 밝기 조정 팀 컬러 + 점선

**구현 방법**:

1. **색상 유틸 함수** — `frontend/src/utils/colorUtils.ts` 신설:
   ```typescript
   /** hex 6자리 색상을 amount(0~1)만큼 흰색 방향으로 밝게 조정 */
...
2. **동팀 여부 판단** — `App.tsx`:
   ```tsx
   const sameTeam = driverA && driverB &&
...
3. **라인 스타일 props** — 각 차트 컴포넌트에 `isDashedB?: boolean` 추가:
   ```tsx
   // App.tsx에서 전달
   <TelemetryChart  comparisons={...} isDashedB={!!sameTeam} onHover={...} />
   <PositionChart   allLaps={...}     driverColors={...}  dashedDrivers={sameTeam ? new Set([driverB]) : new Set()} />
   <GapChart        allLaps={...}     driverColors={...}  dashedDrivers={sameTeam ? new Set([driverB]) : new Set()} />
   <SectorDeltaChart ... colorB={`#${colorB}`} />
   ```

4. **수정 대상 파일**:
   - `frontend/src/utils/colorUtils.ts` — 신규: `lightenHex()` 유틸
   - `frontend/src/App.tsx` — `sameTeam` 감지, `colorB` 조정, props 전달
   - `frontend/src/components/TelemetryChart.tsx` — `isDashedB` prop → series[1] lineStyle.type 적용
   - `frontend/src/components/PositionChart.tsx` — `dashedDrivers: Set<string>` prop → 해당 드라이버 dashed
   - `frontend/src/components/GapChart.tsx` — 동일
   - `frontend/src/components/TrackMap.tsx` — Driver B 마커 심볼 변경 (`circle` → `diamond`) 또는 테두리 점선
...
*최종 업데이트: 2026-04-13 | Claude Code | Step 10 완료 (레이스 결과 화면 + 드라이버 정렬 개선), Step 11 대기 중*
