...
```
현재 작업 없음 — Step 6 통합 테스트부터 시작하면 된다.
```

### 다음 AI가 할 일 (Step 6 통합 테스트)

1. **데이터 검증**: 2025 R1 세션(FP1~R) 전 드라이버 telemetry rows 확인
2. **API 엔드포인트 검증**: Swagger UI(`http://localhost:8000/docs`)에서 각 엔드포인트 호출
3. **UI 통합 테스트**: 브라우저에서 다양한 세션/드라이버/랩 조합 차트 확인
4. **엣지 케이스 확인**: 삭제된 랩, 데이터 없는 랩, 팀 색상 없는 드라이버 등
