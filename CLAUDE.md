...
| 도구 | 역할 | 특징 |
|------|------|------|
| **Claude Code** (VSCode 익스텐션) | 파일 편집, 명령 실행, 코드 생성, 수학 모델 초안 | VSCode 내 사이드바에서 대화형으로 사용 |
| **Antigravity / Gemini 3.1 Pro** (독립 실행형 앱) | 수학·물리 모델 검증, 브라우저 조작, UI 확인 | Antigravity에서 Gemini 3.1 Pro 모델로 실행 |
...
## 🔬 수학·물리 모델 이중 검증 규칙 (D항목 전용)

> D항목(분석 모델)은 수학적 정합성이 필요하므로 **Claude Code + Gemini 3.1 Pro 양쪽 모두의 합의** 없이는 배포에 포함할 수 없다.

### 역할 분담

| 역할 | 담당 AI | 책임 범위 |
|------|---------|----------|
| **모델 초안 작성** | Claude Code | 공식 선택, TypeScript 구현, 단위 추정 |
| **수학·물리 검증** | Gemini 3.1 Pro (Antigravity) | 공식 정확성, 단위 일관성, 파라미터 타당성 검토 |
| **수정 구현** | Claude Code | Gemini 의견 반영하여 코드 수정 |
| **UI 최종 확인** | Antigravity / Gemini 3.1 Pro | 브라우저에서 결과가 물리적으로 타당한지 최종 확인 |
...
Claude Code 초안 구현
        ↓
Gemini 3.1 Pro 수학 검토
  → 수정 필요: 의견을 검증 계획 문서에 기록
  → 승인:      "Gemini 검증 통과" 표시
        ↓
Claude Code 수정 구현 (필요 시)
        ↓
Antigravity UI 검증 (물리적 타당성 시각 확인)
        ↓
양쪽 모두 통과 시 → 배포 포함 확정
```
...
각 D항목은 아래 조건을 **모두** 충족해야 "검증 완료"로 간주한다:

- [ ] **Gemini 수학 검증**: 공식 단위 일관성 확인, 파라미터 근거 제시
- [ ] **물리적 타당성**: 알려진 F1 레이스 데이터와 대조 시 기대 범위 내 결과
- [ ] **UI 시각 검증**: 직관에 반하는 패턴 없음
- [ ] **TypeScript 컴파일**: 에러 없음
- [ ] **합의 기록**: `docs/completed/` 에 양쪽 검증 결과 기록
