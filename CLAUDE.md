...
## ⚠️ 이중 AI 협업 규칙 (필독 — 모든 작업 전 확인)

이 프로젝트는 **Claude Code + Antigravity(Gemini 3.1 Pro)** 두 AI가 역할을 분담하여 개발합니다.  
**코드 변경은 반드시 아래 협업 규칙을 따릅니다.**
...
| AI | 도구 | 핵심 책임 |
|----|------|----------|
| **Claude Code** | VSCode 익스텐션 | 기획·설계, 코드 작성·수정, ETL 실행, 문서화, git 관리 |
| **Gemini 3.1 Pro** | Antigravity 앱 | 계획 승인, 수학·물리 검증, UI/브라우저 검증, 정합성 확인 |

---

### 2. 변경 분류 및 승인 요건

모든 작업은 아래 세 등급으로 분류하며, **등급에 따라 승인 절차가 다르다.**

| 등급 | 해당 작업 예시 | 승인 요건 |
|------|--------------|----------|
| **🟢 Minor** | 오탈자 수정, 스타일 조정, 주석 추가, 문서 업데이트 | Claude Code 단독 진행 가능 |
| **🟡 Major** | 새 기능 추가, 기존 로직 변경, API 엔드포인트 추가, DB 스키마 변경 | **Gemini 계획 승인 후** 구현 |
| **🔴 Critical** | 수학·물리 분석 모델, 배포 관련 인프라 변경, 보안 관련 수정 | **Gemini 승인 필수 + 검증 기록** |

---

### 3. Major·Critical 작업 표준 워크플로우

```
① Claude Code: 계획 작성
   → CLAUDE.md "현재 진행 중인 작업" 섹션에 계획 기록
   → 변경 범위, 이유, 예상 파일 목록 명시
        ↓
② Gemini 3.1 Pro: 계획 검토
   → CLAUDE.md에 승인(✅) 또는 수정 요청(🔄) 기록
   → 수정 요청 시: 구체적 이유와 대안 제시
        ↓
③ Claude Code: 승인 확인 후 구현
   → Gemini 승인 없이 Major·Critical 코드 변경 금지
   → 구현 완료 후 TypeScript 컴파일·테스트 확인
        ↓
④ Gemini 3.1 Pro: 결과 검증
   → UI 동작 확인(브라우저), 수학 모델은 물리적 타당성 확인
   → 최종 검증 결과를 CLAUDE.md 또는 docs/에 기록
        ↓
⑤ Claude Code: git commit & push
   → 커밋 메시지에 "Gemini 승인: [날짜]" 명시
```
...
Claude Code가 Major 이상 작업을 시작할 때 아래 양식을 `현재 진행 중인 작업` 섹션에 작성한다.

```
### [작업명] — 승인 요청 (YYYY-MM-DD)
- 작성: Claude Code
- 등급: 🟡 Major / 🔴 Critical
- 변경 이유: ...
- 변경 범위: [파일 목록 및 변경 내용 요약]
- 예상 위험: ...

Gemini 검토:
- [ ] 승인 ✅  / 수정 요청 🔄
- 의견: _(Gemini가 직접 기록)_
```
...
3. **작업 완료 시 선점 해제 + CLAUDE.md 갱신.**
4. **CLAUDE.md가 최종 진실(Source of Truth)이다.**

---

### 6. 수학·물리 모델 특별 규칙 (🔴 Critical 자동 분류)

분석 모델(D항목 등)은 Critical로 자동 분류되며 아래를 추가로 충족해야 한다:

- **Gemini 단위 검증(Dimensional Analysis)**: 공식의 단위 일관성 서면 확인
- **물리적 타당성**: 알려진 실제 F1 데이터와 대조 결과 기대 범위 내
- **파라미터 근거**: 경험적 상수는 문헌 또는 논리적 근거 제시
- **합의 기록**: `docs/completed/`에 양쪽 검증 결과 보존

---

### 7. 의견 불일치 해소 원칙

- Claude Code와 Gemini 의견이 다를 경우: **Gemini 의견 우선** (검증자 권한)
- Gemini가 승인 불가로 판단한 기능은 사용자(프로젝트 오너)에게 에스컬레이션
- 사용자 최종 결정 후 CLAUDE.md에 의사결정 이유 기록
...
> Phase 1.5 (Step 9~11) 전체 완료. 다음 단계는 Phase 2 퍼블릭 배포입니다.
> 배포 체크리스트: `docs/plan/plan.md` → "Phase 2 배포 체크리스트" 참조.
...
```
D항목 Gemini 승인 완료, Claude Code 수정 구현 완료 (2026-04-20).
4개 항목 모두 TypeScript 컴파일 통과. Antigravity UI 물리적 타당성 재확인 대기 중.
검증 계획: docs/in-progress/phase1_75_D_validation_plan.md
```

### Phase 1.75: 배포 전 분석 고도화 (C·D 항목)
...
**D항목 (분석 모델, 수학적 검증 필요):**
- ✅ D-2: DRS 델타 팀별 비교 (커밋: `88c8066`)
- ✅ D-4: 트레일 브레이킹 구간 표시 (커밋: `e017242`)
- ✅ D-3: 횡가속도(Lateral G) 히트맵 (커밋: `3b7ba23`)
- ✅ D-1: 연료 보정 페이스 곡선 (커밋: `f68a629`)

### C항목 검증 결과 (Antigravity, 2026-04-16)

- ✅ C-2: Air/Track/Humidity/Wind 헤더 정상, 강우 세션 Rain 파란 텍스트 확인
- ✅ C-1: Speed 패널 DRS 초록 밴드 직선 구간에만 표시, 코너 미표시 확인
- ✅ C-4: SC/VSC/RED 구간 4패널 전체 노란 밴드 정상

**Antigravity UX 피드백 (백로그):**
- Session Selector → Searchable Dropdown 교체 제안
- RaceControlTimeline "!" 레이블에 툴팁 추가 제안

### Step 11 완료 내역 (2026-04-14)

| 파일 | 변경 내용 |
|------|----------|
| `etl/fetch_circuit_info.py` | **신규**: `session.get_circuit_info()` → corners/marshal_sectors JSON (NaN 안전 처리) |
| `etl/fetch_race_control.py` | **신규**: `session.race_control_messages` → dict 리스트 |
| `etl/load_data.py` | `insert_circuit_info()` / `insert_race_control()` 추가, laps-only 포함 통합, `messages=True` 추가 |
| `backend/app/routers/circuits.py` | **신규**: `GET /circuits/{circuit_key}` |
| `backend/app/routers/race_control.py` | **신규**: `GET /race-control?session_id=X` |
| `backend/app/main.py` | circuits / race_control 라우터 등록 |
| `frontend/src/types/f1.ts` | `CircuitInfo`, `CircuitCorner`, `RaceControlMessage` 인터페이스 추가 |
| `frontend/src/api/f1Client.ts` | `fetchCircuit()`, `fetchRaceControl()` 추가 |
| `frontend/src/hooks/useCircuit.ts` | **신규**: `useCircuit()` 훅 (staleTime: Infinity) |
| `frontend/src/hooks/useRaceControl.ts` | **신규**: `useRaceControl()` 훅 |
| `frontend/src/components/TrackMap.tsx` | `circuitInfo` prop 추가, 2D에 코너 번호 레이블 scatter 오버레이 |
| `frontend/src/components/RaceControlTimeline.tsx` | **신규**: SC/VSC/플래그 랩 기반 타임라인 바 |
| `frontend/src/App.tsx` | `useCircuit` / `useRaceControl` 연결, TrackMap·RaceControlTimeline에 전달 |
| `docs/completed/step11_completed.md` | **신규**: 완료 기록 |

데이터: 2025 시즌 24개 서킷 (평균 16.8 코너), race_control 6,256개 메시지 적재 완료
...
│   ├── fetch_results.py       # session.results → session_results (Step 10)
│   ├── fetch_circuit_info.py  # session.get_circuit_info() → circuits JSON (Step 11)
│   ├── fetch_race_control.py  # session.race_control_messages → race_control_messages (Step 11)
...
│   ├── requirements.txt
│   └── app/
│       ├── main.py              # lifespan, CORS, session_season_cache, 라우터 등록 (6개 라우터)
...
│           ├── results.py       # GET /results (Step 10)
│           ├── circuits.py      # GET /circuits/{circuit_key} (Step 11)
│           └── race_control.py  # GET /race-control (Step 11)
│
└── frontend/               # React 18 + TypeScript + Vite
    ├── package.json        # echarts, echarts-for-react, echarts-gl, tslib 포함
    └── src/
        ├── types/f1.ts     # Session, Driver, Lap(Step 9), TelemetryData, CircuitInfo, RaceControlMessage 등
        ...
        │   ├── TrackMap.tsx                # 3D/2D 트랙 맵 + 코너 오버레이 (Step 11)
        │   ├── SectorDeltaChart.tsx        # 섹터 델타 막대 차트 (Step 9)
        │   ├── TyreStrategyChart.tsx       # 타이어 전략 타임라인 (Step 9)
        │   ├── ResultsTable.tsx            # 레이스 결과 테이블 (Step 10)
        │   ├── PositionChart.tsx           # 랩별 포지션 꺾은선 차트 (Step 10)
        │   ├── GapChart.tsx                # 리더 대비 갭 차트 (Step 10)
        │   ├── RaceControlTimeline.tsx     # SC/VSC/플래그 타임라인 바 (Step 11)
│   ├── DrsAnalysisChart.tsx        # DRS 존별 최고속도 드라이버 비교 (D-2)
│   └── FuelCorrectedChart.tsx      # 연료 보정 페이스 곡선 (D-1)
        ├── App.tsx          # 전체 레이아웃 + 텔레메트리/결과 탭 전환
        ├── main.tsx
        └── index.css
```
...
-- session_results 테이블 (Step 10)
id, session_id, season, driver_code,
classified_position, grid_position, points,
q1_ms, q2_ms, q3_ms, status

-- circuits 테이블 (Step 11)
circuit_key(PK), rotation, corners(JSON), marshal_sectors(JSON)
-- 데이터: 2025 시즌 24개 서킷

-- race_control_messages 테이블 (Step 11)
id, session_id, time_ms, lap_number, category, message, flag, driver_code
-- 데이터: 2025 시즌 6,256개 메시지
```

> ⚠️ `etl/schema.sql`의 laps 정의는 아직 ALTER 전 상태입니다. session_results/circuits/race_control_messages는 schema.sql에 미반영 — 신규 환경 구축 시 직접 CREATE 필요.
...
| `etl/fetch_results.py` | **Step 10** FastF1 `session.results` 수집, DNF 안전 처리 |
| `etl/fetch_circuit_info.py` | **Step 11** `session.get_circuit_info()` → corners/marshal_sectors JSON |
| `etl/fetch_race_control.py` | **Step 11** `session.race_control_messages` → race_control_messages |

### Backend

| 파일 | 역할 |
|------|------|
| `backend/app/main.py` | lifespan, CORS, session_season_cache 초기화, 라우터 6개 등록 |
| `backend/app/database.py` | SQLAlchemy 2.0 async engine + get_db() |
| `backend/app/models.py` | ORM 모델 7개 (Lap 모델 Step 9 컬럼 포함) |
| `backend/app/schemas.py` | Pydantic v2 스키마 (LapOut Step 9 필드, TelemetryData x/y/z, SessionResultOut 포함) |
...
| `backend/app/routers/results.py` | **Step 10** GET /results?session_id=X |
| `backend/app/routers/circuits.py` | **Step 11** GET /circuits/{circuit_key} |
| `backend/app/routers/race_control.py` | **Step 11** GET /race-control?session_id=X |

### Frontend

| 파일 | 역할 |
|------|------|
| `src/types/f1.ts` | Session, Driver, Lap(Step 9), TelemetryData(x/y/z), SessionResult, CircuitInfo, RaceControlMessage |
| `src/api/f1Client.ts` | fetchSessions, fetchDrivers, fetchLaps, fetchAllLaps, fetchTelemetry, fetchResults, fetchCircuit, fetchRaceControl |
| `src/hooks/useSessions.ts` | useQuery: GET /sessions |
| `src/hooks/useDrivers.ts` | useQuery: GET /drivers |
| `src/hooks/useLaps.ts` | useQuery: GET /sessions/{id}/laps + useAllLaps + fastestLap() 헬퍼 |
...
| `src/components/TelemetryChart.tsx` | ECharts 4-grid (속도/스로틀/브레이크/기어), onHover 콜백, isDashedB |
| `src/components/TrackMap.tsx` | ECharts-GL 3D(고도 컬러)+2D 동시 2컬럼, 코너 오버레이(Step 11), 카메라 슬라이더 4개 |
...
| `src/components/PositionChart.tsx` | **Step 10** 랩별 포지션 변화 꺾은선 차트 (dashedDrivers 지원) |
| `src/components/GapChart.tsx` | **Step 10** 리더 대비 누적 갭 차트 (dashedDrivers 지원) |
| `src/components/RaceControlTimeline.tsx` | **Step 11** SC/VSC/플래그 랩 기반 타임라인 바 |
| `src/components/DrsAnalysisChart.tsx` | **D-2** DRS 존별 최고속도 + 속도 이득 그룹 막대 차트 |
| `src/components/FuelCorrectedChart.tsx` | **D-1** 연료 보정 페이스 곡선 (파라미터 슬라이더 포함) |
| `src/App.tsx` | 텔레메트리/결과 탭 전환 UI, 드라이버 정렬, circuitInfo/raceControl 연결 |

---

*최종 업데이트: 2026-04-16 | Claude Code | Phase 1.75 C·D 항목 전체 구현 완료, D항목 검증 대기 중*
