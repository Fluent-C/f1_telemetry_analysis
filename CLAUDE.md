...
```
현재 작업 진행 중 — Phase 2: 3D 트랙 맵(Track Map) 및 텔레메트리 동기화 인터랙티브 UI 개발 시작
(Antigravity가 구현 계획을 수립하고 문서화까지만 수행함)
```

### 다음 AI가 할 일 (Claude Code 인계)

1. **텔레메트리 Z축 컬럼 추가**: `backend/app/models.py` 및 DB 스키마에 `z` 컬럼(Float) 추가.
2. **ETL 파이프라인 수정**: `etl/fetch_telemetry.py` 내부 `lap.get_car_data()`를 `lap.get_telemetry()`로 대체, `x, y, z` 파싱 추가.
3. **전체 데이터 재적재**: `--force` 플래그로 `load_data.py`를 실행하여 기존 데이터에 `X,Y,Z` 값을 포함한 상태로 전부 재적재.
4. **프론트엔드 연동**: 2D(X/Y) 스캐터 맵 및 3D(`echarts-gl`) 트랙 맵을 렌더링하고, 스피드 차트에 마우스 호버(`hoverIndex`) 시 트랙 위의 원형 마커가 실시간 동기화 구현. (Config를 통해 3D 맵의 Z축 과장 비율 조절 기능 지원 필요)
