...
```
Step 9 완료 — 랩 데이터 확장 (섹터/스피드트랩/타이어/피트) 전체 구현 완료.
다음: Step 10 (레이스 결과 화면).
```

### 다음 AI가 할 일 (Step 10부터)

**반드시 `phase1_plan.md`의 "Phase 1.5: 배포 전 기능 확장" 섹션을 먼저 읽고 시작하라.**

진행 순서:
1. **Step 10** (Option B): `session_results` 테이블 생성 → `fetch_results.py` 신설 → `results.py` 라우터 → `ResultsTable.tsx` + `PositionChart.tsx` + `GapChart.tsx`
2. **Step 11** (Option C): `circuits` + `race_control_messages` 테이블 → ETL 신설 → 라우터 → `TrackMap.tsx` 코너 오버레이 + `RaceControlTimeline.tsx`
...
| `etl/load_data.py` | `upsert_laps()` SQL 20개 컬럼으로 확장 |
...
| `src/components/TrackMap.tsx` | ECharts-GL 3D/2D 전환, Z Scale 슬라이더, hoverTimeMs 드라이버 위치 |
...
| `src/App.tsx` | 레이아웃: TyreStrategy → TrackMap → SectorDelta → Telemetry 순 |
